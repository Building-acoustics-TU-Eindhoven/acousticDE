...
Since each octave band is modeled separately in the diffusion equation model, a Butterwoth bandpass filter ensures that the random noise occupies the correct frequency range. This step includes the creation of the filter for the random noise. An order 8 for the bandpass filter ensures that the transition band is sharp enough to isolate the frequency regions while keeping stability.
